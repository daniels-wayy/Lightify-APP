...
class DesktopHomeBody extends StatelessWidget {
  ...
  static const rowDevicesCount = 2;
        ...
        SliverSafeArea(
          ...
          sliver: SliverPadding(
            ...
            sliver: ReorderableSliverList(
              ...
              delegate:
                  ReorderableSliverChildBuilderDelegate(childCount: (groupedDevices.length / 2).ceil(), (_, index) {
                return GridView.builder(
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                    childAspectRatio: 1.8,
                    crossAxisCount: rowDevicesCount,
                    crossAxisSpacing: height(12),
                  ),
                  itemBuilder: (context, itemIndex) {
                    final dataIndex = index * rowDevicesCount + itemIndex;
                    if (dataIndex >= groupedDevices.length) {
                      return const SizedBox.shrink();
                    }
                    final groupName = groupedDevices.keys.toList()[dataIndex];
                    final groupDevices = groupedDevices.values.toList()[dataIndex];
                    return DevicesGroup(
                      ...
                      layoutType: AdaptiveLayoutType.desktop,
                    );
                  },
                  itemCount: rowDevicesCount,
                );
              }),
            ),
          ),
